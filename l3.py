...
# for node 0
y_train = np.loadtxt("data/processed/y_train.txt", dtype=int)[:, 3]
y_test = np.loadtxt("data/processed/y_test.txt", dtype=int)[:, 3]
#
...
history = model.fit(X_train, y_train, batch_size=128, epochs=90, verbose=1, validation_split=0.2)
score = model.evaluate(X_test, y_test, verbose=1)
predict_list = model.predict(X_test, batch_size=128, verbose=1)
y_write(predict_list, "data/store/l3/l3_predict.txt")
y_write_round(predict_list, "data/store/l3/l3_predict_round.txt")
y_predict_round = []
...
l3_con_mat = confusion_matrix(list(y_test), y_predict_round)
...
# save the result
with open("data/store/l3/l3_result.txt", "a+") as r:
    r.write("\n")
    r.write("\n")
    r.write("epoch is 90 \n")
...
model.save("models/l3.h5")

epochs = len(history.history['loss'])
plt.plot(range(epochs), history.history['loss'], label='loss')
plt.plot(range(epochs), history.history['val_loss'], label='val_loss')
plt.legend()
plt.savefig("data/img/l3_grad_90.png")
plt.show()
