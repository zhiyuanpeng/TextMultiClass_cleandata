...
viterbi_result = np.loadtxt("./data/store/viterbi_log_avg_result.txt")
label = np.loadtxt("./data/store/y_test.txt")
...
for i in range(viterbi_result.shape[0]):
    ...
    for j in range(viterbi_result.shape[1]):
        if viterbi_result[i, j] == label[i, j]:
            right_list[j] += 1
            total_right += 1
    if viterbi_result[i, 0] == label[i, 0] and viterbi_result[i, 1] == label[i, 1]:
        l0_l1_num += 1
    if viterbi_result[i, 1] == label[i, 1] and viterbi_result[i, 2] == label[i, 2]:
        l1_l2_num += 1
    if viterbi_result[i, 0] == label[i, 0] and viterbi_result[i, 3] == label[i, 3]:
        l0_l3_num += 1
    if viterbi_result[i, 0] == label[i, 0] and viterbi_result[i, 4] == label[i, 4]:
        l0_l4_num += 1
    if viterbi_result[i, 0] == label[i, 0] and viterbi_result[i, 5] == label[i, 5]:
        l0_l5_num += 1
