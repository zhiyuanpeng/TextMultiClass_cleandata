...
toxic_comments_labels = toxic_comments[["severe_toxic"]]
...
x_write(X_train, "data/store/l1_X_train.txt")
x_write(X_test, "data/store/l1_X_test.txt")
y_write(y_train, "data/store/l1_y_train.txt")
y_write(y_test, "data/store/l1_y_test.txt")
#
...
y_write(predict_list, "data/store/l1_predict.txt")
y_write_round(predict_list, "data/store/l1_predict_round.txt")
right_sum = 0
for index in range(len(predict_list)):
    ...
    if y_predict == int(y_test[index]):
        right_sum += 1
accuracy = right_sum / len(predict_list)

# save the result
with open("data/store/l1_result.txt", "a+") as r:
    r.write("self accuracy is " + str(accuracy) + "\n")
    r.write("score is " + str(score[0]) + "\n")
    r.write("accuracy is " + str(score[1]) + "\n")

model.save("models/l1.h5")
