...
# for node 1
y_train = np.loadtxt("data/processed/y_train.txt", dtype=int)[:, 1]
y_test = np.loadtxt("data/processed/y_test.txt", dtype=int)[:, 1]
#
...
y_write(predict_list, "data/store/l1/l1_predict.txt")
y_write_round(predict_list, "data/store/l1/l1_predict_round.txt")
y_predict_round = []
...
l1_con_mat = confusion_matrix(list(y_test), y_predict_round)
...
# save the result
with open("data/store/l1/l1_result.txt", "a+") as r:
    r.write("\n")
    r.write("\n")
    r.write("epoch is 7 \n")
...
plt.legend()
plt.savefig("data/img/l1_grad_7.png")
plt.show()
