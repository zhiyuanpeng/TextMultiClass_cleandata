...
y_train_list = [y_train[i, 0] for i in range(y_train.shape[0])]
class_weights = class_weight.compute_class_weight('balanced',
                                                  np.unique(np.array(y_train_list)),
                                                  np.array(y_train_list))
# write the X_train, X_test, y_train, y_test
x_write(X_train, "data/store/l0_l1_w_X_train.txt")
x_write(X_test, "data/store/l0_l1_w_X_test.txt")
y_write(y_train, "data/store/l0_l1_w_y_train.txt")
y_write(y_test, "data/store/l0_l1_w_y_test.txt")
#
...
history = model.fit(X_train, y_train, batch_size=128, epochs=10, verbose=1, validation_split=0.2, class_weight=class_weights)
score = model.evaluate(X_test, y_test, verbose=1)
predict_list = model.predict(X_test, batch_size=128, verbose=1)
y_write(predict_list, "data/store/l0_l1_w_predict.txt")
y_write_round(predict_list, "data/store/l0_l1_w_predict_round.txt")
right_sum = 0
...
# save the result
with open("data/store/l0_l1_w_result.txt", "a+") as r:
...
plt.legend()
plt.show()
plt.savefig("l0_l1_w_adam_10.png")
